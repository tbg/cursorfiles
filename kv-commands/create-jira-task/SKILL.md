...
1. **Preferred: Use a numeric sprint ID** - Invoke `jira-task` with `sprints` to list available sprints and pick the appropriate ID
...
- You MAY invoke `jira-task` with `sprints` for sanity checks (it has no side effects)
